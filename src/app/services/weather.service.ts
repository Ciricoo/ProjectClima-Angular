...
export class WeatherService {
  ...
  fetchData(): Observable<Weather> {
    ...
    return this.http
      .get<Weather>(
        `https://api.hgbrasil.com/weather?format=json-cors&key=8ba9e74a&city_name=${this.selectedCity}`
      )
      .pipe(
        /**Encadeia uma operação de manipulação de dados em um fluxo de dados assíncrono. **/
        tap(
          /** Executa uma função de efeito colateral em cada item do fluxo de dados, sem modificá-lo. **/ (
            data
          ) => {
